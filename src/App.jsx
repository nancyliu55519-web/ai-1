...
function computeBaziPrecise(inputs) {
  ...
  // —— 十神（天干十神，日干为「我」）——优先自算（确定性、可靠），库方法作补充 ——
  const TEN_GOD_STEM_WX = { 甲: "木", 乙: "木", 丙: "火", 丁: "火", 戊: "土", 己: "土", 庚: "金", 辛: "金", 壬: "水", 癸: "水" };
  const STEM_YIN_YANG = { 甲: "阳", 乙: "阴", 丙: "阳", 丁: "阴", 戊: "阳", 己: "阴", 庚: "阳", 辛: "阴", 壬: "阳", 癸: "阴" };
  const WX_SHENG = { 木: "火", 火: "土", 土: "金", 金: "水", 水: "木" }; // 生
  const WX_KE = { 木: "土", 土: "水", 水: "火", 火: "金", 金: "木" };   // 克
  function tenGod(dayGan, otherGan) {
    if (!dayGan || !otherGan) return "";
    const dw = TEN_GOD_STEM_WX[dayGan], ow = TEN_GOD_STEM_WX[otherGan];
    const same = STEM_YIN_YANG[dayGan] === STEM_YIN_YANG[otherGan];
    if (ow === dw) return same ? "比肩" : "劫财";
    if (WX_SHENG[ow] === dw) return same ? "偏印" : "正印";   // 生我者印
    if (WX_SHENG[dw] === ow) return same ? "食神" : "伤官";   // 我生者食伤
    if (WX_KE[ow] === dw) return same ? "七杀" : "正官";     // 克我者官杀
    if (WX_KE[dw] === ow) return same ? "偏财" : "正财";     // 我克者财
    return "";
  }
  const _dayGan = pillars.day ? pillars.day.charAt(0) : "";
  const shiShenGan = {
    year: pillars.year ? tenGod(_dayGan, pillars.year.charAt(0)) : "",
    month: pillars.month ? tenGod(_dayGan, pillars.month.charAt(0)) : "",
    day: "日主",
    hour: pillars.hour ? tenGod(_dayGan, pillars.hour.charAt(0)) : null,
  };
  // 藏干：优先用库，取不到则用固定藏干表
  const BRANCH_HIDE = { 子: ["癸"], 丑: ["己", "癸", "辛"], 寅: ["甲", "丙", "戊"], 卯: ["乙"], 辰: ["戊", "乙", "癸"], 巳: ["丙", "庚", "戊"], 午: ["丁", "己"], 未: ["己", "丁", "乙"], 申: ["庚", "壬", "戊"], 酉: ["辛"], 戌: ["戊", "辛", "丁"], 亥: ["壬", "甲"] };
  function hideOf(pz) { return pz ? (BRANCH_HIDE[pz.charAt(1)] || []) : []; }
  let hideGan = {
    year: hideOf(pillars.year),
    month: hideOf(pillars.month),
    day: hideOf(pillars.day),
    hour: inputs.hourUnknown ? null : hideOf(pillars.hour),
  };
  try {
    hideGan = {
      ...
      hour: inputs.hourUnknown ? null : eightChar.getTimeHideGan(),
    };
  } catch (e) { /* 用上面自算的藏干表 */ }
  // 藏干十神：基于藏干自算
  function hideTenGods(arr) { return Array.isArray(arr) ? arr.map((g) => `${g}=${tenGod(_dayGan, g)}`) : []; }
  const shiShenZhi = {
    year: hideTenGods(hideGan.year),
    month: hideTenGods(hideGan.month),
    day: hideTenGods(hideGan.day),
    hour: hideGan.hour ? hideTenGods(hideGan.hour) : null,
  };

  // —— 五行统计（天干+地支藏干，用于粗判旺衰）——
  const WX_OF_STEM = { 甲: "木", 乙: "木", 丙: "火", 丁: "火", 戊: "土", 己: "土", 庚: "金", 辛: "金", 壬: "水", 癸: "水" };
  const wuxingCount = { 木: 0, 火: 0, 土: 0, 金: 0, 水: 0 };
  const dayStem = pillars.day ? pillars.day.charAt(0) : "";
  const dayWx = WX_OF_STEM[dayStem] || "";
  try {
    // 天干计数
    for (const pz of [pillars.year, pillars.month, pillars.day, pillars.hour]) {
      if (!pz) continue;
      const s = pz.charAt(0);
      if (WX_OF_STEM[s]) wuxingCount[WX_OF_STEM[s]] += 1;
    }
    // 藏干计数
    if (hideGan) {
      for (const arr of [hideGan.year, hideGan.month, hideGan.day, hideGan.hour]) {
        if (!Array.isArray(arr)) continue;
        for (const g of arr) if (WX_OF_STEM[g]) wuxingCount[WX_OF_STEM[g]] += 1;
      }
    }
  } catch (e) {}

  // —— 地支六冲、六合、三合（客观关系，供AI直接引用，不让它自己推）——
  const branches = [pillars.year, pillars.month, pillars.day, pillars.hour]
    .filter(Boolean).map((pz) => pz.charAt(1));
  ...
  const relChong = [], relHe = [];
  for (let i = 0; i < branches.length; i++) {
    for (let j = i + 1; j < branches.length; j++) {
      if (LIU_CHONG[branches[i]] === branches[j]) relChong.push(`${branches[i]}${branches[j]}相冲`);
      if (LIU_HE[branches[i]] === branches[j]) relHe.push(`${branches[i]}${branches[j]}六合`);
    }
  }
  const branchRelations = { chong: relChong, he: relHe, presentBranches: branches };
...
- 用户问什么就直接答什么，一次把话讲清楚讲完整，别挤牙膏、别东拉西扯、别老让人追问。回答可以充实但要紧扣问题，把该说的一次说到位。
- 尽量不要反问用户。信息够就直接断；只有在完全没法判断时（比如连问的是谁、什么事都不知道）才简短问一句，其余情况一律直接给答案。
...
- 如果用户问的是具体的活动、出行或运动安排（比如"明天滑雪怎么样""周末去爬山顺不顺""今天适合练车吗""去健身有没有收获"等），照常依据卦象/盘面来断，但要把结论落到这件事的具体场景上：结合卦象吉凶谈谈——那天的状态精力如何、是否适合去、适合大胆尝试还是求稳、要留意的方面（比如安全、身体状态、天气雪况路况、同行的人、时间点等），以及大概能有什么收获或该注意别勉强。像滑雪这类有一定风险的运动，卦象若显示不顺，就温和提醒当天量力而行、注意安全、别硬上高难度，不必吓唬人也不说死。始终从卦象出发来分析，而不是凭常识空谈。
  ...
  switch (systemId) {
    ...
    case "bazi": {
      ...
      const placeLine = (extra.birthPlace || extra.currentPlace)
        ...
        : "";
      // —— 算法算好的客观事实块（硬约束，禁止AI自己现推十神/藏干/冲合）——
      let factBlock = "";
      try {
        const sg = r.shiShenGan, hg = r.hideGan, wc = r.wuxingCount, rel = r.branchRelations;
        const pillLabel = ["年", "月", "日", "时"];
        const pillGan = [p.year && p.year.charAt(0), p.month && p.month.charAt(0), p.day && p.day.charAt(0), p.hour && p.hour.charAt(0)];
        const pillZhi = [p.year && p.year.charAt(1), p.month && p.month.charAt(1), p.day && p.day.charAt(1), p.hour && p.hour.charAt(1)];
        const ganShiShen = sg ? [sg.year, sg.month, "日主", sg.hour] : [];
        let lines = [];
        lines.push(`日主：${r.dayStem}（${r.dayWx}）——命局以此为「我」，所有十神都相对日主而定。`);
        if (sg) {
          const parts = [];
          for (let i = 0; i < 4; i++) {
            if (!pillGan[i]) continue;
            parts.push(`${pillLabel[i]}干${pillGan[i]}=${ganShiShen[i] || "?"}`);
          }
          lines.push(`天干十神（已算好，直接用）：${parts.join("，")}。`);
        }
        if (hg) {
          const hgParts = [];
          const hgArr = [hg.year, hg.month, hg.day, hg.hour];
          for (let i = 0; i < 4; i++) {
            if (!pillZhi[i] || !Array.isArray(hgArr[i])) continue;
            hgParts.push(`${pillLabel[i]}支${pillZhi[i]}藏[${hgArr[i].join("、")}]`);
          }
          if (hgParts.length) lines.push(`地支藏干（已算好，直接用）：${hgParts.join("；")}。`);
        }
        if (wc) {
          lines.push(`五行分布（含藏干计数）：木${wc.木}、火${wc.火}、土${wc.土}、金${wc.金}、水${wc.水}。据此判日主旺衰，别凭空说。`);
        }
        if (rel) {
          const relTxt = [];
          if (rel.chong && rel.chong.length) relTxt.push(`地支相冲：${rel.chong.join("、")}`);
          else relTxt.push("四柱地支之间无相冲");
          if (rel.he && rel.he.length) relTxt.push(`地支六合：${rel.he.join("、")}`);
          relTxt.push(`原局出现的地支只有：${(rel.presentBranches||[]).join("、")}（未出现的地支不得用于三合/三会/冲合，不得无中生有）`);
          lines.push(relTxt.join("；") + "。");
        }
        factBlock = "【客观事实·已由算法精确算好，必须严格遵守，不得自行改算】\\n" + lines.join("\\n") + "\\n";
      } catch (e) {}
      let nowLine = "";
      try {
      ...
      return (
        ...
        `${dayunLines}\n` +
        `全程基于这些数据来聊，不要改动干支、不要重新排盘。${placeLine}${nowLine}\n` +
        factBlock +
        `【十神铁律——务必遵守，这是最容易出错的地方】十神一律以日主为参照，用上面算好的结果，绝对不要自己现推：生我者为正印/偏印，我生者为伤官/食神，克我者为正官/七杀，我克者为正财/偏财，同我者为比肩/劫财（阴阳同为偏/枭/杀/劫，阴阳异为正印/食神/正官/正财等）。谈某柱十神时，只用上面「天干十神」「地支藏干」里已列出的，不得凭印象改叫别的名字。谈冲合只用上面「地支相冲/六合」里列出的，原局没有的地支不得用来凑三合局。判旺衰只依据上面的五行分布。\n` +
        `断法参酌 ${BAZI_CLASSICS.slice(0, 5).join("、")} 等传统命理体系。\n\n` +
        `【八字解读流程——你的第一条回复必须严格按下面几步走，用清晰的小标题分段，内容要详实、有料，不要简略】\n` +
        `第一步「核验往事」：根据此八字的大运流年、十神，推算此人过去真实可能发生过的事，列出恰好 10 条，按人生阶段分组（童年0-12岁、少年13-18岁、青年19-30岁、成年31岁至今，各阶段分配若干条，合计10条）。【年龄换算铁律】此人出生公历年就是 0 岁那年（如1979年出生，则0-12岁对应1979-1991年，不要把年柱干支所属年份当成出生年）。每条务必给一个明确的年龄段并换算成对应公历年份，前后自洽。每条都要写得具体、有画面感、可核对——不只是「学业有波动」，而要像「大约X岁前后，学习上遇到一次明显的挫折或转折，可能是转学、成绩下滑或与老师同学关系紧张」这样有情节、有方向。方面可涉及：学业、健康疾病伤灾、搬家迁移、父母长辈家庭变故、感情起伏分合、财运升降、事业工作转折、贵人小人等。\n` +
...
.hist-group-head{width:100%;display:flex;align-items:center;gap:8px;background:rgba(201,161,90,.07);border:1px solid var(--line2);border-radius:9px;padding:11px 12px;cursor:pointer;transition:border-color .15s}
.hist-group-head:hover{border-color:var(--gold-lt2)}
.hist-group-caret{color:var(--gold-lt);font-size:11px;width:12px}
.hist-group-name{flex:1;text-align:left;font-family:var(--serif);font-size:15px;color:var(--gold-lt)}
.hist-group-count{font-family:var(--mono);font-size:11px;color:var(--ink-sub);background:rgba(0,0,0,.25);border-radius:10px;padding:1px 9px}
.hist-group .drawer-item{margin:6px 0 0 10px}
.drawer-item-q{font-size:12px;color:var(--ink-sub);line-height:1.4;margin-top:4px;overflow:hidden;text-overflow:ellipsis;display:-webkit-box;-webkit-line-clamp:1;-webkit-box-orient:vertical}
                ...
                <div className="drawer-list">
                  {(() => {
                    ...
                    for (const h of historyList) {
                      const key = h.systemName || "其他";
                      if (idxMap[key] === undefined) { idxMap[key] = groups.length; groups.push({ key, items: [] }); }
                      groups[idxMap[key]].items.push(h);
                    }
                    const firstQuestion = (h) => {
                      const m = (h.messages || []).find((x) => x.role === "user");
                      return m ? m.content : (h.messages && h.messages.length ? "" : "（还没提问）");
                    };
                    return groups.map((g) => {
                      const open = !!openHistGroups[g.key];
                      return (
                        <div className="hist-group" key={g.key}>
                          <button className="hist-group-head" onClick={() => setOpenHistGroups((p) => ({ ...p, [g.key]: !p[g.key] }))}>
                            <span className="hist-group-caret">{open ? "▾" : "▸"}</span>
                            <span className="hist-group-name">{g.key}</span>
                            <span className="hist-group-count">{g.items.length}</span>
                          </button>
                          {open && g.items.map((h) => (
                            <div className={"drawer-item" + (currentHistoryRef.current && currentHistoryRef.current.id === h.id ? " cur" : "")} key={h.id}>
                              <button className="drawer-item-main" onClick={() => { resumeHistory(h); setShowHistory(false); }}>
                                ...
                                <div className="drawer-item-q">{firstQuestion(h)}</div>
